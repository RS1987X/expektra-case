...
- Follow-up action: align Part 3 forecast contract naming with current recursive behavior; `ExogenousFallbackSteps` currently represents different concepts by model (baseline key-miss fallback vs FastTree recursive steps beyond anchor). Add explicit per-model-safe counters (for example `SeasonalKeyFallbackSteps` and `RecursiveStepsBeyondAnchor`) and keep backward compatibility via a migration plan for CSV/consumers.
...
- Follow-up action: the pipeline is single-threaded (uses ~13% CPU on an 8-core machine). Consider parallelizing independent work: train BaselineSeasonal and FastTreeRecursive concurrently, run Part4 and Diagnostics in parallel, or use `Parallel.ForEach` for independent per-model/per-anchor computations. This would significantly reduce wall-clock time on multi-core machines.
