...
- Follow-up action: add explicit timestamp deduplication in preprocessing (or fail-fast duplicate validation), with a deterministic policy (for example keep-last per `utcTime`) and audit/summary counts for removed duplicates.
