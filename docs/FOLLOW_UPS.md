...
- Follow-up action: exclude imputed `Target` rows from validation metrics and persist a per-row audit artifact (for example flags like `IsValidation`, `IsTargetImputed`) so scoring is reproducible and inspectable.
